...
def main_view(request):
    ...
    if request.user.is_authenticated:
        assortiment = Markets_prods.objects.select_related("prod_id").filter(
    ...
    return render(
        request,
        "marketapp/index.html",
        context={"assortiment": assortiment, "title": title},
    )
...
def reg(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            return render(
                request, "marketapp/signin.html", context={"form": form, "title": title}
            )
    else:
        form = RegForm()
        return render(
...
def product(request, id):
    ...
    return render(
        request,
        "marketapp/product.html",
        context={"product_info": product, "title": title},
    )
    ...
    if request.user.is_staff == False:
        ...
        if request.method == "POST":
            ...
            if form.is_valid():
                count_duplicates = len(
                    Clients_prods.objects.filter(
                        client_id=request.user.id, product_id=product_id
                    )
                )
                ...
                Clients_prods.objects.create(
                    pay=0,
                    count=count,
                    client_id=Shopper.objects.get(id=request.user.id),
                    market_id=Markets.objects.get(id=market_id),
                ...
                return render(
                    request,
                    "marketapp/product_buy.html",
                    context={"product_info": product, "form": form, "title": title},
                )
        else:
            form = Form_buy()
            return render(
        ...
        if request.method == "POST":
            ...
            if form.is_valid():
                ...
                if new_count > 0:
                    old_data = Markets_prods.objects.get(
                        prod_id=product_id, market_id=request.user.market_id
                    )
...
def shopping_cart(request):
    title = "Корзина"
    products = Clients_prods.objects.select_related("product_id").filter(
        client_id=request.user.id
    )
    if request.method == "GET":
        return render(
            request,
            "marketapp/shopping_cart.html",
            context={"own_cart": products, "title": title},
        )
    else:
        try:
            ...
            with connection.cursor() as cursor:
                cursor.execute(
                    f"call client_buy_own_shopping_cart_products({request.user.id})"
                )
...
def add_product(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            count = form.cleaned_data["count"]
            try:
                m_prods = Markets_prods.objects.get(
                    market_id=request.user.market_id, prod_id=prod_id
                )
                if m_prods.id > 0:
                    print("Невозможно добавить дупликат товара")
                    return HttpResponseRedirect(reverse("market:index"))
            except:
                Markets_prods.objects.create(
                    market_id=Markets.objects.get(id=request.user.market_id),
                    prod_id=Products.objects.get(id=prod_id),
                    count=count,
                )
                return HttpResponseRedirect(reverse("market:index"))
        else:
...
def client_orders(request):
    title = "Заказы"
    orders = Clients_orders.objects.select_related("client_id").filter(
    ...
    return render(
        request,
        "marketapp/client_orders.html",
        context={"client_orders": orders, "order_products": prods, "title": title},
    )
...
def market_orders(request):
    ...
    return render(
        request,
        "marketapp/orders_for_manager.html",
        context={"market_orders": orders, "title": title},
    )
